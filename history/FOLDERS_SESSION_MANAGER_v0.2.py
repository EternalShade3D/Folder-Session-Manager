...
from PySide6.QtWidgets import QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QLabel, QCheckBox, QSpinBox, QListWidget, QFileDialog, QTreeWidget, QTreeWidgetItem, QToolBar, QToolTip, QMessageBox
...
CONFIG = {
    ...
    "texts": {
        ...
        "close_windows_cb": "Close Existing Windows (Caution!)",
        ...
        "confirm_close": "Confirm Close Windows",
        ...
        "yes_backup": "Yes, with Backup",
        "yes_no_backup": "Yes, without Backup",
        "no": "No",
    },
    "truncate_length": 30,
}
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Barra de ferramentas para controle de visualização (direita)
        view_toolbar = QToolBar("View Controls")
        view_toolbar.setStyleSheet("QToolBar { background-color: transparent; border: none; spacing: 5px; }")
        ...
        compact_action = view_toolbar.addAction("Toggle Compact View")
        compact_action.triggered.connect(self.toggle_compact_view)
        self.addToolBar(Qt.RightToolBarArea, view_toolbar)

        # Barra de ferramentas para ações de sessão (inferior)
        action_toolbar = QToolBar("Session Actions")
        action_toolbar.setStyleSheet("QToolBar { background-color: transparent; border: none; spacing: 5px; }")
    ...
    def wheelEvent(self, event):
        widget_under_cursor = self.childAt(event.position().toPoint())
            ...
            try:
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    for entry in session_data:
                        ...
                        print(f"Restored window {hwnd} at ({x}, {y}) with size ({width}, {height})")
            except Exception as e:
