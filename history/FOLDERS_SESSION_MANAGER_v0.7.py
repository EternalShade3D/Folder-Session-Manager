...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QLabel,
                              ...
                              QMenuBar, QMessageBox, QGridLayout, QToolTip, QLineEdit)
from PySide6.QtCore import Qt, QEvent, Signal, QSettings, QPoint
from PySide6.QtGui import QColor, QClipboard, QFontMetrics, QIcon, QKeySequence, QStandardItemModel, QStandardItem
...
CONFIG = {
    ...
    "sizes": {
        "min": (500, 400), "base": (600, 400), "font": (12, 14), "padding": 10, "button_min_width": 100,
...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # Menu de Temas
        menu_bar = QMenuBar()
        theme_menu = menu_bar.addMenu(CONFIG["texts"]["theme_menu"])
        theme_menu.addAction(CONFIG["texts"]["dark_theme"], lambda: self.set_theme("dark"))
        theme_menu.addAction(CONFIG["texts"]["light_theme"], lambda: self.set_theme("light"))
        main_layout.addWidget(menu_bar)

        # Título
        title_label = QLabel(CONFIG["texts"]["app_title"], objectName="title_label")
        ...
        settings_layout.addWidget(QLabel(CONFIG["texts"]["timeout_label"]))
    ...
    def apply_theme(self):
        ...
        self.setStyleSheet(f"""
            ...
            QLabel#title_label {{ font-size: {int(font_l * 1.2)}pt; font-weight: bold; color: {styles['accent']}; margin-bottom: {padding}px; }}
            QLabel#section_label {{ font-size: {font_l}pt; font-weight: bold; }}
            QCheckBox {{ font-size: {font_m}pt; }}
            QSpinBox, QLineEdit {{ background: {styles['input_bg']}; color: {styles['text']}; border: 1px solid {styles['accent']}; font-size: {font_m}pt; padding: 3px; }}
                ...
                if f.endswith('.json') and f.startswith('explorer_session'):
                    try:
                        with open(os.path.join(self.session_dir, f), 'r', encoding='utf-8') as file:
                            json.load(file)  # Verifica se o JSON é válido
    ...
    def expand_all(self):
        ...
        self.status_bar.showMessage("All items expanded", 3000)

    def collapse_all(self):
        self.preview_tree.collapseAll()
        self.status_bar.showMessage("All items collapsed", 3000)

    def clear_preview(self):
        self.preview_tree.clear()
        self.search_input.clear()
        self.item_count_label.setText(CONFIG["texts"]["item_count"].format(count=0))
        self.status_bar.showMessage("Preview cleared", 3000)
    ...
    def filter_preview(self, text):
        ...
        for i in range(self.preview_tree.topLevelItemCount()):
            ...
            for j in range(item.childCount()):
                child = item.child(j)
                if text in child.text(0).lower() or text in child.text(1).lower():
                ...
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    for entry in session_data:
                        ...
                        for path in entry['paths']:
                            ...
                            time.sleep(self.timeout_spin.value() / 100.0)
