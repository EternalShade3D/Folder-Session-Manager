...
import win32gui
import win32api  # Added to fix NameError
...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QLabel,
                              ...
                              QMenu, QMessageBox, QProgressDialog)  # Added QProgressDialog
from PySide6.QtCore import Qt, QEvent, Signal, QSettings, QPoint, QDateTime
from PySide6.QtGui import QColor, QClipboard, QFontMetrics, QKeySequence, QWheelEvent
...
CONFIG = {
    ...
    "sizes": {
        "fixed": (1200, 600), "min": (1200, 500), "font": (12, 16), "padding_prop": 0.02,
...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.save_button.setToolTip("Save current session automatically 💾")
    ...
    def apply_theme(self):
        ...
        self.setStyleSheet(f"""
            ...
            QLabel#section_label {{ font-size: {font_l}pt; font-weight: bold; text-transform: uppercase; }}
            ...
            try:
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    for entry in session_data:
                        ...
                        # Open a new File Explorer instance for this hwnd with retry
                        ...
                        explorer_hwnd = None
                        for _ in range(3):  # Retry loop for window detection
                            for window in shell.Windows():
                        ...
                        if explorer_hwnd is None:
                            self.status_bar.showMessage(f"❌ Failed to find window for hwnd {hwnd}", 5000)
                            continue

                        # Move the window to its original position
                        win32gui.MoveWindow(explorer_hwnd, x, y, width, height, True)
                        time.sleep(0.5)

                        # Add tabs for additional paths with retry logic
                        for path in paths[1:]:
                            if os.path.exists(path):
                                max_retries = 2
                                for attempt in range(max_retries + 1):
                                    try:
                                        ...
                                        time.sleep(max(1.0, self.timeout_spin.value() / 100.0))
                                        self.status_bar.showMessage(
                                            f"{CONFIG['texts']['restored_window'].format(hwnd=hwnd)} in tab", 3000)
                                        break
                                    except Exception as e:
                                        if attempt < max_retries:
                                            self.status_bar.showMessage(f"❌ Retry {attempt + 1}/{max_retries} for path {path}: {e}", 3000)
                                            time.sleep(1.0)
                                        else:
                                            ...
                                            if reply == QMessageBox.Yes:
                                                shell.Explore(path)
                                                self.status_bar.showMessage(f"📂 Opened {path} manually", 3000)
                                            elif reply == QMessageBox.No:
                                                self.status_bar.showMessage(f"⏭️ Skipped {path}", 3000)
                                            else:
                                                self.status_bar.showMessage("🛑 Restore cancelled by user", 3000)
                                                progress.close()
                                                return
                            else:
                                self.status_bar.showMessage(CONFIG["texts"]["invalid_path"].format(path=path), 5000)
                            ...
                            if win32api.GetAsyncKeyState(0x1B):
                                self.status_bar.showMessage("🛑 Restore stopped by user", 3000)
                                progress.close()
                                return
                        time.sleep(1.0)
